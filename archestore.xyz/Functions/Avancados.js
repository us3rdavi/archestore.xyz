...
async function Avançados(interaction, client) {
    ...
    container.addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
            `## ${Emojis.get('_support_emoji')} Painel de Proteção\n` +
...
async function Emojis24(interaction, client) {
    ...
    container.addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
            ...
            `Gerencie os emojis personalizados utilizados pelo bot. Para saber para qual função cada emoji serve, consulte a imagem de referência.`
        )
    );
...
async function Perms24(interaction, client) {
    ...
    container.addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
            `## ${Emojis.get('_staff_emoji')} Configuração de Permissão\n` +
