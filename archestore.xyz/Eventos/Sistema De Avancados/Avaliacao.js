    ...
    run: async (interaction, client) => {
        ...
        if (customId.startsWith("feedbackModal_")) {
            ...
            if (!feedbackChannelId24) {
                return await interaction.reply({ content: `${Emojis.get('negative_emoji')} Canal de feedback não configurado.`, ephemeral: true });
            }

            const feedbackChannel24 = await client.channels.fetch(feedbackChannelId24).catch(() => null);

            if (!feedbackChannel24) {
                return await interaction.reply({ content: `${Emojis.get('negative_emoji')} Canal de feedback não encontrado ou inacessível.`, ephemeral: true });
            }
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    {
                        name: `${Emojis.get('store_emoji')} **Nota**`,
                        value: `**${nota24}/05**`,
                        inline: true
                    },
                    {
                        name: `${Emojis.get('_messages_emoji')} **Feedback**`,
                        value: `**\`${feedback}\`**`,
                        inline: true
                    },
                    {
                        name: `${Emojis.get('_people_emoji')} **Assumido por**`,
                        value: `**${assumidoPor}**`,
                        inline: true
                    }
                )
            ...
            await interaction.update({ content: `${Emojis.get('confirmed_emoji')} Avaliação enviada com sucesso!`, components: [] });
        }
    },
};
